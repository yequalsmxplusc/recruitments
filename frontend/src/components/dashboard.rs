...
pub fn dashboard(props: &Props) -> Html {
    ...
    if let Some(err) = &*error {
        return html! {
            <div class="min-h-screen flex items-center justify-center theme-wrapper">
                <div class="text-red-500 text-lg">{ err }</div>
            </div>
        };
    }
    ...
    let Some(applicant) = &*applicant else {
        return html! {
            <div class="min-h-screen flex items-center justify-center theme-wrapper">
                <p class="theme-text-primary text-lg">{ "Loading…" }</p>
            </div>
        };
    };
    if applicant.is_admin {
        return html! { <AdminDashboard auth={props.auth.clone()} /> };
    }
                        ...
                        <div class="px-4 py-5 sm:px-6">
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                                <div class="sm:col-span-1">
                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Full name" }</dt>
                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary font-semibold">{ &applicant.name }</dd>
                                </div>
                                <div class="sm:col-span-1">
                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Email address" }</dt>
                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary font-semibold">{ &applicant.email }</dd>
                                </div>

                                {
                                    if applicant.round.as_deref() == Some("Interview") {
                                        html! {
                                            <>
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Mobile Number" }</dt>
                                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary">{ applicant.mobile.as_deref().unwrap_or("N/A") }</dd>
                                                </div>
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Graduation Year" }</dt>
                                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary">{ applicant.grad_year.as_deref().unwrap_or("N/A") }</dd>
                                                </div>
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Gender" }</dt>
                                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary">{ applicant.gender.as_deref().unwrap_or("N/A") }</dd>
                                                </div>
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Faculty & Department" }</dt>
                                                ...
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Skill" }</dt>
                                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary">{ applicant.skill.as_deref().unwrap_or("N/A") }</dd>
                                                </div>
                                                <div class="sm:col-span-1">
                                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Consideration Round" }</dt>
                                                    <dd class="mt-1 text-sm sm:text-base theme-text-primary">{ applicant.round.as_deref().unwrap_or("N/A") }</dd>
                                                </div>
                                                {
                                                    if let Some(slot) = applicant.interview_slot.as_deref().filter(|s| !s.is_empty()) {
                                                        html! {
                                                            <div class="sm:col-span-2 bg-blue-500/20 border border-blue-500 p-4 rounded-lg">
                                                                <dt class="text-sm sm:text-base font-semibold text-blue-600 dark:text-blue-400">{ "Selected Interview Slot" }</dt>
                                                                <dd class="mt-1 text-lg font-bold text-blue-700 dark:text-blue-300">{ format_datetime(slot) }</dd>
                                                            </div>
                                                        }
                                                    } else {
                                                        html! {}
                                ...
                                <div class="sm:col-span-1">
                                    <dt class="text-sm sm:text-base font-medium opacity-75">{ "Application Status" }</dt>
                                ...
                                </div>
                                {
                                    if applicant.status.as_deref() == Some("Selected") {
                                        html! {
                                            <div class="sm:col-span-1">
                                                <a href="https://chat.whatsapp.com/groupchat" target="_blank"
                                                   class="bg-green-500 text-white py-2 px-4 rounded-lg shadow-lg hover:bg-green-600 inline-block transition-colors duration-200">
                                                    { "Join WhatsApp Group" }
                                                </a>
                                            </div>
                                        }
                                ...
                                <div class="sm:col-span-2 text-center mt-6">
                                    {
                                        if has_booked {
                                            ...
                                            html! {
                                                <Link<Route> to={Route::Rounds}
                                                    classes="bg-blue-600 text-white py-3 px-6 rounded-lg shadow-lg hover:bg-blue-700 inline-block font-semibold transition-colors duration-200">
                                                    { "Go to My Rounds" }
                                                </Link<Route>>
                                            }
                                        }
